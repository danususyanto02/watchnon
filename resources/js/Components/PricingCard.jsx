...
export default function Pricing({ isPremium, id, name, price, durationInMonth, features, onSelectSubscription }) {
    return (
        <>
            <div className="mx-auto max-w-screen hidden lg:block">
                {!isPremium &&
                    <div className="flex flex-col gap-[30px] py-[30px] px-7 outline outline-1 outline-[#F1F1F1] rounded-[26px] text-black w-[300px] h-[max-content]">
                        ...
                        <div>
                            <PrimaryButton onClick={onSelectSubscription}
                                type="button" variant="white-outline">
                                <span className="text-base">Start {name}</span>
                            </PrimaryButton>
                        </div>
                    </div>}
                {isPremium && <div className="flex justify-center gap-10 mt-[70px]">
